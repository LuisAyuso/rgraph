...
impl Graph {
    ...
    pub fn bind_asset(&mut self, src: &str, sink: &str) -> Result<(), GraphError>{
        ...
        if !self.nodes.values().find(|node| node.get_ins().iter().find(|name| name.as_str()==sink).is_some() ).is_some(){
            return Err(GraphError::UndefinedAssetSlot(sink.into()));
        }
        
        if !self.nodes.values().find(|node| node.get_outs().iter().find(|name| name.as_str()==src).is_some() ).is_some(){
...
impl<'a, 'b> GraphSolver<'a, 'b> {
    ...
    pub fn execute_terminals(&mut self) -> Result<SolverStatus, SolverError> {
        let tmp : Vec<&NodeRunner> = self.graph.get_terminals().iter().map(|x| x.as_ref() ).collect();
...
mod tests {
    ...
    fn graph_with_assets() {
        ...
        //do connection
        g.bind_asset("gen_one :: one", "plus_one :: one").expect("binding must be doable");
        g.bind_asset("plus_one :: plusone", "the_one_task :: plusone").expect("binding must be doable");
        g.bind_asset("gen_one :: one", "the_one_task :: one").expect("binding must be doable");

        g.get_binding("plus_one :: one").expect("binding must be set");
        g.get_binding("the_one_task :: plusone").expect("binding must be set");
        g.get_binding("the_one_task :: one").expect("binding must be set");
        ...
        for _ in 0..10 {
            ...
            solver.get_value::<f32>("the_one_task :: last_value").expect("could not retrieve result");
        }

        assert!(cache.get_value::<f32>("the_one_task :: last_value").expect("must be f32") == 3f32);
    }
    ...
    fn terminals(){
        ...
        g.bind_asset("no_input :: i", "sink_1 :: input").expect("binding must be doable");
        g.bind_asset("no_input :: i", "sink_2 :: name").expect("binding must be doable");
        ...
        assert!(cache.get_value::<u32>("no_input :: i").expect("must be f32") == 1234);
    }
